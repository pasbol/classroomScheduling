...
$(document).ready(function(){
	var theme = 'classic';
	var url = '<?php echo base_url().''.$manager; ?>/scheduleManagementJSON/';
    var source =
    {
        datatype: 'json',
        datafields: [
	...
	var initRoomSchedule =  function (index, parentElement, gridElement, record) {
		...
		if (grid != null) { 
			grid.jqxGrid({
				source: scheduleManagementInitJSON(roomID), 
                theme: theme, 
                altrows: true,               
				...
				renderstatusbar: function (statusbar) {
					...
					addButton.click(function (event) {
						...
						var timeLabel = $('<label style="float: left; width: 35%; text-align: center;">Time: </label>');
						var timeInput = $('<div style="margin: 3px;" id="timeInput"></div>');
						...
						windowContent.append(timeLabel);
						windowContent.append(timeInput);
						...
						okButton.bind('click', function(event) {
							...
							postData.push($('#timeInput').val());
							postData.push(now);

							for(var i = 0; i < checkedItems.length; i++){
								var url = '<?php echo base_url() .''.$manager; ?>/scheduleManagementInsert/';
						...
						container.jqxWindow({                    
							title: 'Insert Schedule for '+room+' - '+description, 
							resizable: false, 
							theme: theme, 
							width: 450,                        
							isModal: true,
		            ...
		            deleteButton.click(function (event) {
						...
						var dataContent = $('<p>Do you really want to delete? ['+rowdata['sc_id']+' | '+rowdata['sc_code']+' '+rowdata['subject_title']+' '+rowdata['time']+' '+rowdata['day']+' ]</p>');
						var okButton = $('<input type="button" style="margin-top: 15px; float: right;" value="OK" />');
						var cancelButton = $('<input type="button" style="margin-top: 15px; margin-left: 5px; float: right;" value="Cancel"/>');
						...
						okButton.bind('click', function(event) {
							...
							var url = '<?php echo base_url().''.$manager; ?>/scheduleManagementDelete/';
				...
				columns: [
					{ text: 'SC_ID', datafield: 'sc_id', pinned: true},
					{ text: 'SC CODE', datafield: 'sc_code', pinned: true},
					{ text: 'Descriptive Title', datafield: 'subject_title', pinned: true},
					{ text: 'Day', datafield: 'day', filtertype: 'list', pinned: true},
					{ text: 'Time', datafield: 'time', pinned: true},
					{ text: 'Department', datafield: 'department_code', filtertype: 'list'},
					{ text: 'Semester', datafield: 'semester', filtertype: 'list'},
					{ text: 'Generated', datafield: 'generated'}
				]
			});
			
            grid.bind("bindingcomplete", function (event) {
                grid.jqxGrid('autoresizecolumn', 'semester');
            	grid.jqxGrid('autoresizecolumn', 'sc_id');            	            
            	grid.jqxGrid('autoresizecolumn', 'sc_code');
                grid.jqxGrid('autoresizecolumn', 'subject_title'); 
                grid.jqxGrid('autoresizecolumn', 'time'); 
	...
	function scheduleManagementInitJSON(roomID){
		var url = '<?php echo base_url().''.$manager; ?>/scheduleManagementScheduleInitJSON/'+roomID;
		var source =  {
			datatype: 'json',
			datafields: [
